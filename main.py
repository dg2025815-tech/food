...
plt.rcParams['font.family'] = 'Malgun Gothic'  # Windows 기준 (Mac일 경우 'AppleGothic')
plt.rcParams['axes.unicode_minus'] = False

# 1. 학교 정보 설정 (서울시교육청: B10)
SCHOOL_MAP = {
    "1": ("성남고등학교", "7010193"),
    "2": ("당곡고등학교", "7010073"),
    "3": ("수도여자고등학교", "7010090")
...
def plot_calorie_chart(df, school_name, year, month):
    ...
    bars = plt.bar(avg_cal.index, avg_cal.values, color='#ff9f43', edgecolor='#ee5253')
    
    # 막대 위에 숫자 표시
    for bar in bars:
        height = bar.get_height()
        plt.text(bar.get_x() + bar.get_width()/2., height + 5, f'{height:.1f} kcal', ha='center', va='bottom', fontsize=10)

    plt.title(f"{school_name} - {year}년 {month}월 요일별 평균 칼로리", fontsize=14, fontweight='bold')
    plt.xlabel("요일", fontsize=11)
    plt.ylabel("평균 칼로리 (Kcal)", fontsize=11)
    plt.ylim(0, max(avg_cal.values) + 150)
    plt.grid(axis='y', linestyle='--', alpha=0.5)
    
    print("\n📊 요일별 평균 칼로리 그래프를 출력합니다... (그래프 창을 닫으면 프로그램이 계속 진행됩니다)")
    plt.show()

# 6. 메인 실행 부분
def main():
    print("=" * 50)
    print("🍱 고등학교 월별 급식 달력 & 칼로리 분석기")
    print("=" * 50)
    ...
    school_choice = input("학교 번호를 선택하세요 (기본 1): ").strip() or "1"
    school_name, school_code = SCHOOL_MAP.get(school_choice, SCHOOL_MAP["1"])
    
    now = datetime.now()
    year_input = input(f"연도를 입력하세요 (기본 {now.year}): ").strip() or str(now.year)
    month_input = input(f"월을 입력하세요 (기본 {now.month}): ").strip() or str(now.month)
    
    year = int(year_input)
    month = int(month_input)
    
    api_key = input("나이스 API Key (없으면 엔터): ").strip()
    ...
    print(f"\n⏳ {school_name}의 {year}년 {month}월 급식 정보를 조회 중입니다...")
    df = fetch_monthly_meal(school_code, start_ymd, end_ymd, api_key)
    
    if df.empty:
        print("\n⚠️ 해당 기간에 급식 정보가 없거나 방학/휴교 기간입니다.")
        return
    
    # 1) 요일별 평균 칼로리 그래프 띄우기
    plot_calorie_chart(df, school_name, year, month)
    
    # 2) 터미널에 달력 형태로 메뉴 출력
    print(f"\n==================== 📅 {year}년 {month}월 급식 달력 ====================")
